...
function POD(S, ϵ=1e-5, norm_matrix=nothing)
  ...
  if !(norm_matrix === nothing)
    if !issparse(norm_matrix)
      norm_matrix = sparse(norm_matrix)
    end

    H = cholesky(norm_matrix).U
    mul!(S, H, S)
  end
  ...
  while cumulative_energy / total_energy < 1.0 - ϵ * 2 && N < size(S)[2] - 1
...
function get_parametric_specifics(ROM_info, μ_nb)
  ...
  elseif ROM_info.case === 1
    α(x) = μ_nb[3] + 1 / μ_nb[3] * exp(-((x[1] - μ_nb[1])^2 + (x[2] - μ_nb[2])^2) / μ_nb[3])
    return parametric_specifics(μ_nb, [], α, [], [], [])
