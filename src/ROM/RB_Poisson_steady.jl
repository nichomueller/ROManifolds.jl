  ...
  if "A" in operators
    ...
    if ROM_info.problem_nonlinearities["A"] === false
      ...
      projection_time += @elapsed begin
        ...
        (RB_variables.Aₙ_affine, RB_variables.Aₙ_idx) = DEIM_offline(Aₙ_i, ROM_info.ϵₛ ^ 2)
        if ROM_info.save_offline_structures
          save_CSV(RB_variables.Aₙ_affine, joinpath(ROM_info.paths.ROM_structures_path, "Aₙ_affine.csv"))
          save_CSV(RB_variables.Aₙ_idx, joinpath(ROM_info.paths.ROM_structures_path, "Aₙ_idx.csv"))
        end
  ...
  if "A" in operators
    ...
    if !ROM_info.problem_nonlinearities["A"]
      ...
      projection_time += @elapsed begin
        ...
        Aₙ_i[:, i_nₛ] = reshape((A_iΦₛᵘ)' * RB_variables.Pᵘ_inv * A_iΦₛᵘ, :, 1)
        end
      end
    ...
    if !ROM_info.problem_nonlinearities["f"] && !ROM_info.problem_nonlinearities["h"]
      ...
      projection_time += @elapsed begin
        ...
        if !ROM_info.problem_nonlinearities["A"]
          ...
          nₛ_MDEIM = min(50, ROM_info.nₛ)
          @info "S-PGRB: The forcing term is affine, but the LHS isn't: must run MDEIM offline phase on $nₛ_DEIM snapshots"
          Fₙ_i = zeros(RB_variables.nₛᵘ, nₛ_MDEIM)
          for i_nₛ = 1:nₛ_MDEIM
            μ_i = parse.(Float64, split(chop(μ[i_nₛ]; head=1, tail=1), ','))
            parametric_info = get_parametric_specifics(ROM_info, μ_i)
            FE_space = get_FE_space(problem_info, parametric_info.model)
            A_i = assemble_stiffness(FE_space, ROM_info, parametric_info)
            A_iΦₛᵘ = A_i * RB_variables.Φₛᵘ
            Fₙ_i[:, i_nₛ] = (A_iΦₛᵘ)' * RB_variables.Pᵘ_inv * F
          end
      ...
      projection_time += @elapsed begin
        ...
        if !ROM_info.problem_nonlinearities["A"]
          ...
          for i_nₛ = 1:min(50, ROM_info.nₛ)
            μ_i = parse.(Float64, split(chop(μ[i_nₛ]; head=1, tail=1), ','))
            parametric_info = get_parametric_specifics(ROM_info, μ_i)
            F_i = assemble_forcing(FE_space, parametric_info)
            Fₙ_i[:, i_nₛ] = AΦₛᵘPᵘ_inv * F_i
          end
          ...
          for i_nₛ = 1:min(50, ROM_info.nₛ)
            ...
            F_i = assemble_forcing(FE_space, parametric_info)
            A_iΦₛᵘ = A_i * RB_variables.Φₛᵘ
            Fₙ_i[:, i_nₛ] = (A_iΦₛᵘ)' * RB_variables.Pᵘ_inv * F_i
          end
...
function get_RB_system(ROM_info, RB_variables::RBProblem, param; FE_space = nothing)
  ...
  if ROM_info.problem_nonlinearities["A"] === false
    ...
    Aₙ_μ = (RB_variables.Φₛᵘ)' * assemble_stiffness(FE_space, ROM_info, param) * RB_variables.Φₛᵘ
...
function test_MDEIM_Manzoni(ROM_info, RB_variables, FE_space, nₛ_MDEIM, μ, param_nbs)
  ...
  DEIM_mat, DEIM_idx = DEIM_offline(S, ϵ)
  ...
  time_MDEIM_on = @elapsed begin
    for i in param_nbs
      ...
      A_sparse = assemble_matrix(∫(∇(FE_space.ϕᵥ) ⋅ (parametric_info.α * ∇(FE_space.ϕᵤ))) * dΩ_sparse, FE_space.V, FE_space.V₀)
      _, A_affine_sparse = MDEIM_online(A_sparse, DEIM_mat, DEIM_idx)
...
function test_my_MDEIM(ROM_info, RB_variables, nₛ_MDEIM, μ, param_nbs)
  ...
  time_MDEIM_on = @elapsed begin
    for i in param_nbs
      ...
      FE_space = get_FE_space(problem_info, parametric_info.model)
      A = assemble_stiffness(FE_space, problem_info, parametric_info)
      Aₙ = reshape((RB_variables.Φₛᵘ)' * A * RB_variables.Φₛᵘ, :, 1)
      _, Aₙ_affine = MDEIM_online(A, Aₙ, Aₙ_idx)
      err += abs.(A - RB_variables.Φₛᵘ * Aₙ_affine * (RB_variables.Φₛᵘ)')
    end
  end
