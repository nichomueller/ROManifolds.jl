...
function get_Aₙ(ROM_info::Info, RB_variables::PoissonSTPGRB) :: Vector

  get_Aₙ(ROM_info, RB_variables.S)

end

function get_Mₙ(ROM_info::Info, RB_variables::PoissonSTPGRB) :: Vector
  ...
  if isfile(joinpath(ROM_info.paths.ROM_structures_path, "Mₙ.csv")) && isfile(joinpath(ROM_info.paths.ROM_structures_path, "MΦ.csv")) && isfile(joinpath(ROM_info.paths.ROM_structures_path, "MΦᵀPᵤ⁻¹.csv"))
    ...
    RB_variables.MΦ = reshape(MΦ,RB_variables.S.Nₛᵘ,RB_variables.S.nₛᵘ,:)
...
function get_MAₙ(ROM_info::Info, RB_variables::PoissonSTPGRB) :: Vector
...
function assemble_MAₙ(RB_variables::PoissonSTPGRB)
  ...
  MAₙ = zeros(RB_variables.S.nₛᵘ,RB_variables.S.nₛᵘ,RB_variables.Qᵐ*RB_variables.Qᵃ)
  [MAₙ[:,:,(i-1)*nₛᵘ+j] = RB_variables.MΦ'[:,:,i] * RB_variables.S.AΦᵀPᵤ⁻¹'[:,:,j] for i=1:nₛᵘ for j=1:nₛᵘ]
  RB_variables.MAₙ = MAₙ

end

function assemble_affine_matrices(ROM_info::Info, RB_variables::PoissonSTPGRB, var::String)
  ...
  if var == "M"
    ...
    RB_variables.Mₙ = zeros(RB_variables.S.nₛᵘ, RB_variables.S.nₛᵘ, RB_variables.Qᵐ)
    RB_variables.Mₙ[:,:,1] = (M*RB_variables.S.Φₛᵘ)' * RB_variables.Pᵤ⁻¹ * (M*RB_variables.S.Φₛᵘ)
    RB_variables.MΦ = zeros(RB_variables.S.Nₛᵘ, RB_variables.S.nₛᵘ, RB_variables.Qᵐ)
    RB_variables.MΦ[:,:,1] = M*RB_variables.S.Φₛᵘ
  else
...
function assemble_MDEIM_matrices(ROM_info::Info, RB_variables::PoissonSTPGRB, var::String)
  ...
  if var == "M"
    ...
    for q = 1:RB_variables.Qᵐ
      RB_variables.MΦ[:,:,q] = reshape(Vector(MDEIM_mat[:, q]), RB_variables.S.Nₛᵘ, RB_variables.S.Nₛᵘ) * RB_variables.S.Φₛᵘ
    end
...
function assemble_affine_vectors(ROM_info::Info, RB_variables::PoissonSTPGRB, var::String)
...
function assemble_DEIM_vectors(ROM_info::Info, RB_variables::PoissonSTPGRB, var::String)
...
function assemble_offline_structures(ROM_info::Info, RB_variables::PoissonSTPGRB, operators=nothing)
...
function save_affine_structures(ROM_info::Info, RB_variables::PoissonSTPGRB)
...
function get_affine_structures(ROM_info::Info, RB_variables::PoissonSTPGRB) :: Vector
    ...
    for i_t = 1:RB_variables.nₜᵘ
      ...
      for j_s = 1:RB_variables.S.nₛᵘ
        for j_t = 1:RB_variables.nₜᵘ
          ...
          term1 = δt^2*RB_variables.S.Aₙ[i_s,j_s,:]'*( θ^2*Φₜᵘ_A[i_t,j_t,:] + (1-θ)^2*θ^2*Φₜᵘ₋₁₋₁_A[i_t,j_t,:] + θ*(1-θ)*Φₜᵘ₋₁₁_A[i_t,j_t,:] + θ*(1-θ)*Φₜᵘ₁₋₁_A[i_t,j_t,:] )
...
function get_RB_RHS_blocks(ROM_info::Info, RB_variables::PoissonSTPGRB, θᶠ, θʰ)
  ...
  Qᶠ_tot = RB_variables.S.Qᶠ*(RB_variables.Qᵐ+RB_variables.S.Qᵃ)
  Qʰ_tot = RB_variables.S.Qʰ*(RB_variables.Qᵐ+RB_variables.S.Qᵃ)
  δt = ROM_info.δt
  θ = ROM_info.θ
  δtθ = δt*θ
  nₜᵘ = RB_variables.nₜᵘ
  ...
  for i_s = 1:RB_variables.S.nₛᵘ
    for i_t = 1:RB_variables.nₜᵘ
      ...
      term1 = RB_variables.S.Fₙ[i_s,1:Qᵐᶠ]'*Φₜᵘ_F[i_t,1:Qᵐᶠ] + RB_variables.S.Hₙ[i_s,1:Qᵐʰ]'*Φₜᵘ_H[i_t,1:Qᵐʰ]
      term2 = δtθ*(RB_variables.S.Fₙ[i_s,Qᵐᶠ+1:end]'*Φₜᵘ_F[i_t,Qᵐᶠ+1:end] + RB_variables.S.Hₙ[i_s,Qᵐʰ+1:end]'*Φₜᵘ_H[i_t,Qᵐʰ+1:end])
      term3 = -θ*( RB_variables.S.Fₙ[i_s,1:Qᵐᶠ]'*Φₜᵘ₋₁₁_F[i_t,1:Qᵐᶠ] + RB_variables.S.Hₙ[i_s,1:Qᵐʰ]'*Φₜᵘ₋₁₁_H[i_t,1:Qᵐʰ] )
      term4 = δtθ*(1-θ)*(RB_variables.S.Fₙ[i_s,Qᵐᶠ+1:end]'*Φₜᵘ₋₁₁_F[i_t,Qᵐᶠ+1:end] + RB_variables.S.Hₙ[i_s,Qᵐʰ+1:end]'*Φₜᵘ₋₁₁_H[i_t,Qᵐʰ+1:end])
...
function get_RB_system(ROM_info::Info, RB_variables::PoissonSTPGRB, param)
...
function build_param_RHS(ROM_info::Info, RB_variables::PoissonSTPGRB, param, θᵐ, θᵃ)
  ...
  AΦᵀPᵤ⁻¹ = assemble_online_structure(θᵃ_temp, RB_variables.S.AΦᵀPᵤ⁻¹)
  RHSΦₜ = RHS*RB_variables.Φₜᵘ
  RHSΦₜ₁ = RHS[:,2:end]*RB_variables.Φₜᵘ[1:end-1,:]

  RHSₙ = (MΦᵀPᵤ⁻¹+δtθ*AΦᵀPᵤ⁻¹)*RHSΦₜ + θ*(δt*(1-θ)*AΦᵀPᵤ⁻¹-MΦᵀPᵤ⁻¹)*RHSΦₜ₁

  push!(RB_variables.S.RHSₙ, RHS)

end

function get_θ(ROM_info::Info, RB_variables::PoissonSTPGRB, param) :: Tuple
