...
function compress_array(entire_array::BlockMatrix)
  ...
  nza = map(compress_array,entire_array)

  nonzero_idx_blocks = first.(nza)
  nonzero_idx = mortar(nonzero_idx_blocks)
  nonzero_val_blocks = last.(nza)
...
Base.size(nzb::BlockNnzArray,idx...) = map(nza->size(nza.nonzero_val,idx...),nzb)
...
function tpod(nzb::BlockNnzArray{T,N,OT};kwargs...) where {T,N,OT}
  ...
  return BlockNnzArray{OT}(nonzero_val,nza.nonzero_idx,nza.nrows)
end
