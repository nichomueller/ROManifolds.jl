...
GridapDistributed.Algebra.local_views(a::DistributedPTCellField) = a.fields

function CellField(f::AbstractPTFunction,trian::DistributedTriangulation)
...
function evaluate!(cache,f::DistributedPTCellField,x::DistributedCellPoint)
  map(f.fields,x.points) do f,x
    evaluate!(nothing,f,x)
  end
end

function evaluate!(cache,k::Operation,a::DistributedPTCellField)
...
function evaluate!(cache,k::Operation,a::DistributedPTCellField,b::Number)
  fields = map(a.fields) do f
...
function evaluate!(cache,k::Operation,b::Number,a::DistributedPTCellField)
  fields = map(a.fields) do f
...
function evaluate!(cache,k::Operation,a::DistributedPTCellField,b::Function)
  fields = map(a.fields) do f
    evaluate!(nothing,k,f,b)
  end
  DistributedPTCellField(fields)
...
function evaluate!(cache,k::Operation,b::Function,a::DistributedPTCellField)
  fields = map(a.fields) do f
    evaluate!(nothing,k,b,f)
  end
  DistributedPTCellField(fields)
...
GridapDistributed.Algebra.local_views(a::DistributedPTDomainContribution) = a.contribs
