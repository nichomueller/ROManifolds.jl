...
function setup_RB()
  ...
  ROM_info = ROMSpecificsUnsteady(probl_nl, paths, RB_method, time_reduction_technique, perform_nested_POD, considered_snaps, t₀, T, δt, θ, ϵₛ, ϵₜ, use_norm_X, build_parametric_RHS, nₛ_MDEIM, nₛ_DEIM, space_time_M_DEIM, postprocess, import_snapshots, import_offline_structures, save_offline_structures, save_results)
  if RB_method === "ST-GRB"
    RB_variables = setup((0,0,0))
  else
    RB_variables = setup((0,0,0,0))
  end
