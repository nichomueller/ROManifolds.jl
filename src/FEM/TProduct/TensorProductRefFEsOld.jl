...
struct TensorProduct1DVectorValue{T,D,V} <: AbstractArray{VectorValue{D,T},D}
  vv1d::V
  function TensorProduct1DVectorValue(vv1d::NTuple{D,AbstractVector{VectorValue{1,T}}}) where {T,D}
    V = typeof(vv1d)
    new{T,D,V}(vv1d)
  end
end
...
function Base.getindex(a::TensorProduct1DVectorValue{T,D},i::Vararg{Integer,D}) where {D,T}
  ...
  @inbounds for d in 1:D
    p[d] = a.vv1d[d][i[d]].data[1]
  end
  p
end

function nodes_indexing_map(D,order)
  v = CartesianIndices{D}[]
  nodes_indexing_map!(v,D,order)
  v
end
    ...
    for i = 1:D-K+1
      ...
      if K > 1
        for j in i+1:D
          vj = copy(vi)
          vj[j] = D:D
          if length(findall(!iszero,vj)) <= K
            push!(v,CartesianIndices(Tuple(vj)))
          end
        end
      else
        push!(v,CartesianIndices(Tuple(vi)))
      end
    end
  end
end

type_prod(::Type{T},::Val{N}) where {T,N} = @abstractmethod
type_prod(::Type{VectorValue{1,T}},::Val{N}) where {T,N} = VectorValue{N,T}

function tprod(t::NTuple{N,DataType}) where N
  type_prod(first(t),Val(N))
end
...
function tprod(v::NTuple{N,VectorValue{1,T}}) where {N,T}
  collect1d(v)
end
...
function tprod(v::NTuple{N,<:AbstractVector{VectorValue{1,T}}}) where {N,T}
  TensorProduct1DVectorValue(v)
end

struct TensorProductLagrangianDofBasis{P,V,D,B} <: AbstractVector{ReferenceFEs.PointValue{P}}
  bases::NTuple{D,B}
  function TensorProductLagrangianDofBasis(bases::NTuple{D,B}) where {D,P1,V,B<:LagrangianDofBasis{P1,V}}
...
Base.getindex(a::TensorProductLagrangianDofBasis,i::Integer) = ReferenceFEs.PointValue(get_nodes(a)[i])
...
function ReferenceFEs.get_nodes(a::TensorProductLagrangianDofBasis)
  tprod_nodes = tprod(get_nodes.(a.bases))
...
function conformity_check(c)
  @notimplemented "Tensor product refFEs must have same conformity trait"
end

function conformity_check(c::NTuple{N,T}) where {N,T<:Conformity}
  conformity = first(c)
  return conformity
end
...
get_separate_order(r::TensorProductRefFE) = get_order.(r.reffe)

ReferenceFEs.get_order(r::TensorProductRefFE) = minimum(get_orders(r))

get_separate_orders(r::TensorProductRefFE) = get_orders.(r.reffe)

ReferenceFEs.get_orders(r::TensorProductRefFE{D}) where D = ntuple(i->get_order(r.reffe[i]),D)

num_separate_dofs(r::TensorProductRefFE) = num_dofs.(r.reffe)

ReferenceFEs.num_dofs(r::TensorProductRefFE) = prod(num_dofs.(r.reffe))

num_separate_polytope(r::TensorProductRefFE) = get_polytope.(r.reffe)

ReferenceFEs.get_polytope(r::TensorProductRefFE{D}) where D = Polytope(ntuple(i->HEX_AXIS,D)...)

get_separate_prebasis(r::TensorProductRefFE) = get_prebasis.(r.reffe)

ReferenceFEs.get_prebasis(r::TensorProductRefFE{D}) where D = MonomialBasis{D}(get_type(r),get_orders(r),get_terms(r))

# ReferenceFEs.get_dof_basis(r::TensorProductRefFE) = TensorProductLagrangianDofBasis(get_dof_basis.(r.reffe))

get_separate_dof_basis(r::TensorProductRefFE) = get_dof_basis.(r.reffe)

ReferenceFEs.get_dof_basis(r::TensorProductRefFE) = LagrangianDofBasis(get_type(r),get_polytope(r),get_orders(r))
...
function ReferenceFEs.get_face_nodes(r::TensorProductRefFE)
  polytope = get_polytope(r)
  orders = get_orders(r)
  dofs = get_dof_basis(r)
  T = get_type(r)
  if any(map(i->i==0,orders)) && !all(map(i->i==0,orders))
    ...
    TensorProductFaceNodes(T,polytope,orders,dofs)
  end
end
...
function Base.getindex(a::TensorProductFaceDofs{T,D},iface::Integer) where {T,D}
  dim = D
  for d = 0:D-1
    if iface <= get_offset(a.polytope,d)
      dim = d
      break
    end
  end
  face = Polytope{dim}(a.polytope,iface)
  if dim == 0
  ...
  face_own_nodes = _get_face_own_nodes(a.polytope,a.orders)[iface]
  face_own_dofs = _get_face_own_dofs(face_own_nodes,a.dofs.node_and_comp_to_dof)
...
function Base.getindex(a::TensorProductFaceNodes{T,D},i::Integer) where {T,D}
  offsets = get_offsets(a.polytope)
  offsets_minus_i = offsets .- i .+ 1
  dim = findfirst(offsets_minus_i .> 0) - 1
  iface = offsets_minus_i[findfirst(offsets_minus_i .> 0)]
  face = Polytope{dim}(a.polytope,iface)
  if dim == 0
    p0 = Polytope{0}(a.polytope,1)
    refface = LagrangianRefFE(T,p0,())
  ...
  nnodes = length(a.dofs.nodes)

  face_own_nodes = _get_face_own_nodes(a.polytope,a.orders)[iface]

  face_to_num_fnodes = dim == D ? nnodes : num_nodes(refface)
  face_to_lface_to_own_fnodes = dim == D ? face_own_nodes : get_face_own_nodes(refface)
  ...
  for (lface,faceto) in enumerate(lface_to_face)
    own_nodes = face_own_nodes[faceto]
