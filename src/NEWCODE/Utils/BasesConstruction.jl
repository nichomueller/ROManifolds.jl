...
struct StandardPOD end
struct ReducedPOD end

POD(S::AbstractMatrix,args...;ϵ=1e-4,pod_style=ReducedPOD()) = POD(pod_style,S,args...;ϵ)

function POD(::StandardPOD,S::AbstractMatrix,X::SparseMatrixCSC;ϵ=1e-4)
  H = cholesky(X)
  L = sparse(H.L)
  U,Σ,_ = svd(L'*S[H.p,:])
  n = truncation(Σ,ϵ)
...
function POD(::StandardPOD,S::AbstractMatrix;ϵ=1e-4)
  U,Σ,_ = svd(S)
  n = truncation(Σ,ϵ)
  U[:,1:n]
end
