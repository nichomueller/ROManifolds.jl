...
μn = realization(feop,10)
t = get_times(fesolver)

jac((u,p),(du,dp),(v,q)) = get_jacobian(feop)[1](μn,t,(u,p),(du,dp),(v,q))
res((u,p),(v,q)) = get_residual(feop)(μn,t,(u,p),(v,q))

ode_op = get_algebraic_operator(feop)
ode_cache = allocate_cache(ode_op,μn,t)
ode_cache = update_cache!(ode_cache,ode_op,μn,t)
Xh, = ode_cache
dxh = ()
Ns = length(get_free_dof_ids(test))
_xh = PTArray([rand(Ns) for _ = 1:length(t)*length(μn)])
for i in 2:get_order(feop)+1
  dxh = (dxh...,EvaluationFunction(Xh[i],_xh))
end
xh = TransientCellField(EvaluationFunction(Xh[1],_xh),dxh)
du = get_trial_fe_basis(trial(nothing,nothing))
dv = get_fe_basis(test)

dc_jac_old = integrate(old_jac(xh,du,dv))
dc_jac = integrate(jac(xh,du,dv))
# @check all(map(isapprox,dc_jac_old[Ω],dc_jac[Ω]))
test_ptarray(dc_jac_old[Ω],dc_jac[Ω])

dc_res_old = integrate(old_res(xh,dv))
dc_res = integrate(res(xh,dv))
# @check all(map(isapprox,dc_jac_old[Ω],dc_jac[Ω]))
test_ptarray(dc_res_old[Ω],dc_res[Ω])

# old stuff
conv(u,∇u) = (∇u')⋅u
dconv(du,∇du,u,∇u) = conv(u,∇du)+conv(du,∇u)
_c(u,v) = ∫ₚ(v⊙(conv∘(u,∇(u))),dΩ)
_dc(u,du,v) = ∫ₚ(v⊙(dconv∘(du,∇(du),u,∇(u))),dΩ)

_m(u,v) = ∫ₚ(v⋅u,dΩ)
_a(μ,t,(u,p),(v,q)) = ∫ₚ(aμt(μ,t)*∇(v)⊙∇(u),dΩ) - ∫ₚ(p*(∇⋅(v)),dΩ) - ∫ₚ(q*(∇⋅(u)),dΩ)

old_jac_t(μ,t,(u,p),(dut,dpt),(v,q)) = _m(dut,v)
old_jac(μ,t,(u,p),(du,dp),(v,q)) = _a(μ,t,(du,dp),(v,q)) + _dc(u,du,v)
old_res(μ,t,(u,p),(v,q)) = _m(∂ₚt(u),v) + _a(μ,t,(u,p),(v,q)) + _c(u,v)
cc(x) = @error "Something is wrong"
old_feop = NonlinearPTFEOperator(old_res,old_jac,old_jac_t,(cc,cc),pspace,trial,test)
...
function residual!(
  ...
  println("Norm res: $(norm(map(norm,b.array)))")
  b
end
...
# new, wrong
μ = realization(feop,1)
collect_multi_field_solutions(fesolver,feop,μ)

# old, ok
collect_multi_field_solutions(fesolver,old_feop,μ)

#println("Norm jac: $(norm(map(norm,A.array)))")
