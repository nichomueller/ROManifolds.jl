...
function get_structures(
  ...
  FEM_path(var::String) = joinpath(FEMInfo.Paths.structures_path, var)::String
  ...
  function FEM_norm_matrices()
    ...
    norm_Mats = assemble_FEM_matrix(FEMSpace, FEMInfo, μ, norm_operators)
...
function run_param_loop(
  ...
  get_structures(FEMSpace, FEMInfo, μ[1])
  function FEMμₖ(k::Int)
    println("Collecting solution number $k")
    FEMμ(μ[k])
  end

  blocks_to_matrix(Broadcasting(FEMμₖ)(1:FEMInfo.nₛ))::Matrix{Float}
...
function get_FEM_results(FEMInfo::FOMInfo{ID}) where ID
  ...
  FEM_time = @elapsed begin
    uₕ = run_param_loop(FEMInfo, μ)
  end

  save_structures_in_list(
    (uₕ, μ, [FEM_time/FEMInfo.nₛ]),
    ("uₕ", "μ", "FEM_time"),
    FEMInfo.Paths.snap_path)
