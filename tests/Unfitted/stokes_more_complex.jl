...
add_tag_from_tags!(labels,"dirichlet",[1,3,7])#[1,2,3,4,5,6,7]
...
Γg = BoundaryTriangulation(bgmodel;tags="dirichlet")
...
a(μ,(u,p),(v,q)) = (
  ...
  ∫( - v⋅(n_Γg⋅∇(u))*νμ(μ) + (n_Γg⋅∇(v))⋅u*νμ(μ) + (p*n_Γg)⋅v + (q*n_Γg)⋅u )dΓg
)
...
l(μ,(u,p),(v,q)) = (
  ...
  ∫( (n_Γg⋅∇(v))⋅gμ(μ)*νμ(μ) + (q*n_Γg)⋅gμ(μ) )dΓg
)
...
res(μ,(u,p),(v,q)) = a(μ,(u,p),(v,q)) - (∫( ∇(v)⊙∇(gμ_0(μ)) )dΩ_out + ∫( (n_Γ⋅∇(v))⋅gμ_0(μ)*νμ(μ) + (q*n_Γ)⋅gμ_0(μ) )dΓ)
feop_nl = ParamFEOperator(res,jac,pspace,trial,test)
fesolver_nl = FESolver(NewtonSolver(LUSolver();rtol=1e-10,maxiter=20,verbose=true))
rbsolver_nl = RBSolver(fesolver_nl,state_reduction;nparams_res=50,nparams_jac=50)

x, = solution_snapshots(rbsolver_nl,feop_nl;nparams=1)
u1 = flatten_snapshots(x[1])[:,1]
p1 = flatten_snapshots(x[2])[:,1]
r1 = get_realization(x[1])[1]
U1 = param_getindex(trial_u(r1),1)
...
# V1 = FESpace(Ωact,reffe_u;conformity=:H1,dirichlet_tags=["dirichlet"])
# V2 = FESpace(Ωact_out,reffe_u;conformity=:H1,dirichlet_tags=["dirichlet"])
# U1 = ParamTrialFESpace(V1,gμ)
# U2 = ParamTrialFESpace(V2)

# Q1 = FESpace(Ωact,reffe_p;conformity=:H1)
# Q2 = FESpace(Ωact_out,reffe_p;conformity=:H1)
# P1 = ParamTrialFESpace(Q1)
# P2 = ParamTrialFESpace(Q2)

# Y = MultiFieldParamFESpace([V1,V2,Q1,Q2];style=BlockMultiFieldStyle())
# X = MultiFieldParamFESpace([U1,U2,P1,P2];style=BlockMultiFieldStyle())

# aa(μ,(u1,u2,p1,p2),(v1,v2,q1,q2)) = (
#   ∫( νμ(μ)*∇(v1)⊙∇(u1) - p1*(∇⋅(v)) - q1*(∇⋅(u1)) )dΩ +
#   ∫( ∇(v2)⊙∇(u2) - p2*q2 )dΩ_out +
#   ∫( - v1⋅(n_Γ⋅∇(u1))*νμ(μ) + (n_Γ⋅∇(v1))⋅u1*νμ(μ) + (p1*n_Γ)⋅v1 + (q1*n_Γ)⋅u1 )dΓ
# )

# ll(μ,(u,p),(v,q)) = (
#   ∫( νμ(μ)*∇(v1)⊙∇(u1) - p1*(∇⋅(v)) - q1*(∇⋅(u1)) )dΩ +
#   ∫( ∇(v2)⊙∇(gμ_0(μ)) - p1*q2 )dΩ_out +
#   ∫( (n_Γ⋅∇(v1))⋅gμ_0(μ)*νμ(μ) + (q1*n_Γ)⋅gμ_0(μ) )dΓ
# )

# feop = LinearParamFEOperator(ll,aa,pspace,X,Y)
