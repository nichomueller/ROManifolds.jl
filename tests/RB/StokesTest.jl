...
test_path = "$root/results/Stokes/model_circle_2D_coarse"
order = 2
degree = 2*order
...
res(μ,t,(u,p),(v,q)) = ∫ₚ(v⋅∂t(u),dΩ) + ∫ₚ(aμt(μ,t)*∇(v)⊙∇(u),dΩ) - ∫ₚ(p*(∇⋅(v)),dΩ) - ∫ₚ(q*(∇⋅(u)),dΩ)
...
trial_u = TransientTrialParamFESpace(test_u,[g0,g])
...
if load_structures
  ...
  rbrhs,rblhs = load(info,(BlockRBVecAlgebraicContribution{T},
...
rb_solver(info,feop,fesolver,rbspace,rbrhs,rblhs,sols,params)
end
