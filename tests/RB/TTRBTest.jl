...
core,bases = basis,RB.get_basis.((get_trial(pop),get_test(pop)))
ccores = map((a,b...)->RB.compress_core(a,b...),RB.get_cores(core),RB.get_cores.(bases)...)
ccore = RB.multiply_cores(ccores...)
AA = RB.compress_core(basis.cores_space[1],bases[1].cores_space[1],bases[2].cores_space[1])
BB = RB.compress_core(basis.cores_space[2],bases[1].cores_space[2],bases[2].cores_space[2])
CC = RB.compress_core(basis.core_time,bases[1].core_time,bases[2].core_time)
